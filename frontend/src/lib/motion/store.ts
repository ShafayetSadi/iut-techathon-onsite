...
function cancelled(
  commandId: string,
  reason = "Autonomous sequence cancelled.",
): MotionResult {
...
function formatFinalState(
  ...
  const joints = result.finalJoints
    ...
    : "n/a";
  const ee = result.finalEE ? formatVec3(result.finalEE) : "n/a";
  return `joints=${joints} ee=${ee}`;
}
...
export const useMotionStore = create<MotionState>((set, get) => {
  ...
  const logFinalState = (
    ...
    const reason = result.reason ?? result.error ?? "Motion failed.";
    set({ lastError: reason });
    get().pushLog(`[${commandId}] final state -> failed (${reason})`, "error");
    return result;
  };
  ...
  return {
    ...
    dispatch: async (cmd, context = {}) => {
      ...
      if (!gate.ok) {
        logValidation(commandId, false, gate.reason);
      ...
      try {
        switch (cmd.type) {
          case "set_joint": {
            logBackend(commandId, "local joint update");
          ...
          case "jog_joint": {
            ...
            if (!limit.ok) {
              logValidation(commandId, false, limit.reason);
              logBackend(commandId, "skipped");
              set({ status: "error", continuousJogActive: false });
              return fail(commandId, {
                commandId,
          ...
          case "home": {
            logBackend(commandId, "local home");
            get().home();
            break;
          }
          case "stop": {
            logBackend(commandId, "local stop");
            jogCanceller?.();
            set((state) => ({
          ...
          case "move_to": {
            ...
            logBackend(
              commandId,
              `${response.success ? "ok" : "fail"} (${response.reason ?? "IK solve"})`,
          ...
          case "jog_cartesian": {
            ...
            if (!workspace.ok) {
              logValidation(commandId, false, workspace.reason);
              logBackend(commandId, "skipped");
              set({ status: "error", continuousJogActive: false });
              return fail(commandId, {
                commandId,
          ...
          case "enter_pin": {
            ...
            logBackend(
              commandId,
              `${response.success ? "ok" : "fail"} (${response.message})`,
              response.success ? "ok" : "error",
            );
